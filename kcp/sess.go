	...
	for {
		...
		if waitsnd < int(s.kcp.snd_wnd) && waitsnd < int(s.kcp.rmt_wnd) {
			for _, b := range v {
				n += len(b)
				for {
					if len(b) <= int(s.kcp.mss) || s.kcp.stream == 0 {
						s.kcp.Send(b)
						break
					} else {
...
func disconnectPacket(conv uint64) []byte {
	data := bytes.NewBuffer(make([]byte, 0))

	binary.Write(data, binary.BigEndian, uint32(404))
	binary.Write(data, binary.LittleEndian, uint32(conv>>32))
	binary.Write(data, binary.LittleEndian, uint32(conv&uint64(0xffffffff)))
	binary.Write(data, binary.BigEndian, 5)
	binary.Write(data, binary.BigEndian, uint32(423728276)) // 固定标识符
...
func (s *UDPSession) Close() error {
	...
	if once {
		...
		if s.l != nil { // belongs to listener
			// 发送断开连接包
			s.l.conn.WriteTo(disconnectPacket(s.GetConv()), s.l.Addr())
...
func handshakePacket(conv uint64, enet uint32) []byte {
	data := bytes.NewBuffer(make([]byte, 0))

	binary.Write(data, binary.BigEndian, uint32(325))
	binary.Write(data, binary.LittleEndian, uint32(conv>>32))
	binary.Write(data, binary.LittleEndian, uint32(conv&uint64(0xffffffff)))
	binary.Write(data, binary.BigEndian, enet)
	binary.Write(data, binary.BigEndian, uint32(340870469)) // 固定标识符
...
func (l *Listener) packetInput(data []byte, addr net.Addr) {
	...
	var conv uint64
	// Miho-yo握手包处理
	code := binary.BigEndian.Uint32(data[:codeSize])
	enet := binary.BigEndian.Uint32(data[enetStartIndex:enetEndIndex])
	// 格式是否正确
	if code == 255 && enet != 0 {
		// 随机生成conv
		binary.Read(rand.Reader, binary.BigEndian, &conv)
		l.conn.WriteTo(handshakePacket(conv, enet), addr)

		// 创建新会话
		if s == nil && len(l.chAccepts) < cap(l.chAccepts) { // do not let the new sessions overwhelm accept queue
	...
	if decrypted && len(data) >= IKCP_OVERHEAD {
		...
		if fecFlag == typeData || fecFlag == typeParity { // 16bit kcp cmd [81-84] and frg [0-255] will not overlap with FEC type 0x00f1 0x00f2
			...
			if fecFlag == typeData && len(data) >= fecHeaderSizePlus2+IKCP_OVERHEAD {
				conv = uint64(binary.LittleEndian.Uint32(data[fecHeaderSizePlus2:]))
				sn = binary.LittleEndian.Uint32(data[fecHeaderSizePlus2+IKCP_SN_OFFSET:])
				convRecovered = true
			}
		} else {
			// packet without FEC
			conv = binary.BigEndian.Uint64(data)
